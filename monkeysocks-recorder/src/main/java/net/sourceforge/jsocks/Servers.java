...
import net.sourceforge.jsocks.socks.ManipulatedProxyServer;
...
public class Servers {
    ...
    static public void main(String[] args) {
        ...
        final ManipulatedProxyServer recordServer = new ManipulatedProxyServer(auth, ManipulatedProxyServer.ManipulateStat.RECORD);
        final int portF = port;
        final InetAddress localIPF = localIP;
        new Thread() {
            @Override
            public void run() {
                recordServer.start(portF, 5, localIPF);
            }
        }.start();
        ManipulatedProxyServer replayServer = new ManipulatedProxyServer(auth, ManipulatedProxyServer.ManipulateStat.REPLAY);
        replayServer.start(1081, 5, localIP);
    }
    ...
    static void proxyInit(Properties props) {
        ...
        ManipulatedProxyServer.setProxy(proxy);
    }
