...
import net.sourceforge.jsocks.socks.MonitorProxyServer;
...
public class MontitorSOCKS {
   ...
   static public void main(String[] args){
      ...
      MonitorProxyServer server = new MonitorProxyServer(auth);
   ...
   static Properties loadProperties(String file_name){
      ...
      try{
          System.out.println(MontitorSOCKS.class.getClassLoader().getResource(file_name).getFile());
         InputStream fin = MontitorSOCKS.class.getClassLoader().getResourceAsStream(file_name);
         pr.load(fin);
         fin.close();
      }catch(Exception ioe){
          ioe.printStackTrace();
   ...
   */
   static void proxyInit(Properties props){
      ...
      MonitorProxyServer.setProxy(proxy);
   }
   /**
