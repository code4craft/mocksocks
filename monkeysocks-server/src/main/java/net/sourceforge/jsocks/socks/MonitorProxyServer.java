...
*/
public class MonitorProxyServer implements Runnable{
   ...
   private static final Logger LOG = Logger.getLogger(MonitorProxyServer.class);

   static net.sourceforge.jsocks.socks.Proxy proxy;
...
/////////////////////
    ...
    */
   public MonitorProxyServer(ServerAuthenticator auth){
     this.auth = auth;
...
////////////////////
   ...
   MonitorProxyServer(ServerAuthenticator auth, Socket s, String connectionId){
      this.auth  = auth;
      this.sock  = s;
...
/////////////////
   ...
   */
   public static void setProxy(net.sourceforge.jsocks.socks.Proxy p){
      proxy =p;
   ...
   */
   public static net.sourceforge.jsocks.socks.Proxy getProxy(){
      return proxy;
   }
   ...
   public void start(int port,int backlog,InetAddress localIP){
      try{
        ...
        while(true){
          ...
          MonitorProxyServer ps = new MonitorProxyServer(auth,s, connectionId);
          (new Thread(ps)).start();
        }
...
/////////////////
   ...
   private void handleRequest(ProxyMessage msg)
      ...
      if(!auth.checkRequest(msg)) {
        ProxyMessage response = new Socks5Message(
            net.sourceforge.jsocks.socks.Proxy.SOCKS_NOT_ALLOWED_BY_RULESET);
        response.write(out);
        abort();
        throw new SocksException(net.sourceforge.jsocks.socks.Proxy.SOCKS_NOT_ALLOWED_BY_RULESET);
      }
      ...
      if(msg.ip == null){
        if(msg instanceof Socks5Message){
          ...
          throw new SocksException(net.sourceforge.jsocks.socks.Proxy.SOCKS_FAILURE);
      }
      LOG.debug(connectionId + " " + msg);

      switch(msg.command){
        case net.sourceforge.jsocks.socks.Proxy.SOCKS_CMD_CONNECT:
          onConnect(msg);
        break;
        case net.sourceforge.jsocks.socks.Proxy.SOCKS_CMD_BIND:
          onBind(msg);
        break;
        case net.sourceforge.jsocks.socks.Proxy.SOCKS_CMD_UDP_ASSOCIATE:
          onUDP(msg);
        break;
        default:
          throw new SocksException(net.sourceforge.jsocks.socks.Proxy.SOCKS_CMD_NOT_SUPPORTED);
      }
   }
   ...
   private void handleException(IOException ioe){
      ...
      int error_code = net.sourceforge.jsocks.socks.Proxy.SOCKS_FAILURE;

      if(ioe instanceof SocksException)
          error_code = ((SocksException)ioe).errCode;
      else if(ioe instanceof NoRouteToHostException)
          error_code = net.sourceforge.jsocks.socks.Proxy.SOCKS_HOST_UNREACHABLE;
      else if(ioe instanceof ConnectException)
          error_code = net.sourceforge.jsocks.socks.Proxy.SOCKS_CONNECTION_REFUSED;
      else if(ioe instanceof InterruptedIOException)
          error_code = net.sourceforge.jsocks.socks.Proxy.SOCKS_TTL_EXPIRE;

      if(error_code > net.sourceforge.jsocks.socks.Proxy.SOCKS_ADDR_NOT_SUPPORTED || error_code < 0){
          error_code = net.sourceforge.jsocks.socks.Proxy.SOCKS_FAILURE;
      }
   ...
   private void onConnect(ProxyMessage msg) throws IOException{
      ...
      if(msg instanceof Socks5Message){
        response = new Socks5Message(net.sourceforge.jsocks.socks.Proxy.SOCKS_SUCCESS,
                                         s.getLocalAddress(),
                                         s.getLocalPort());
      }else{
   ...
   private void onBind(ProxyMessage msg) throws IOException{
      ...
      if(msg.version == 5)
         response = new Socks5Message(net.sourceforge.jsocks.socks.Proxy.SOCKS_SUCCESS,ss.getInetAddress(),
                                                          ss.getLocalPort());
      else
         response = new Socks4Message(Socks4Message.REPLY_OK,
                                      ss.getInetAddress(),
...
//////////////////
   ...
   private void doAccept() throws IOException{
      ...
      if(msg.version == 5)
         response = new Socks5Message(net.sourceforge.jsocks.socks.Proxy.SOCKS_SUCCESS, s.getInetAddress(),
                                                           s.getPort());
      else
         response = new Socks4Message(Socks4Message.REPLY_OK,
