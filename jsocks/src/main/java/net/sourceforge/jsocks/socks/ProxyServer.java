...
//////////////////
    ...
    private void pipe(InputStream in, OutputStream out,boolean remote) throws IOException {
        ...
        while (len >= 0) {
            try {
                ...
                len = in.read(buf);
                if (remote){
                    LOG.info(connectionId + "pipe " + len + " from " + remote_sock.getInetAddress().getHostName() + ":" + remote_sock.getPort());
                }       else {
                    LOG.info(connectionId + "pipe " + len + " to " + remote_sock.getInetAddress().getHostName() + ":" + remote_sock.getPort());
                }
                lastReadTime = System.currentTimeMillis();
            } catch (InterruptedIOException iioe) {
